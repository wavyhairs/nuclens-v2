...
| `issue_ledger.py` → `issue_ledger.json` | 이슈 주소가 **어디로 갔는지** 기억한다 (LLM 0회). `build_issue_pages` 는 매 빌드마다 `/issue` 를 지우고 다시 만들어서, 이슈가 카탈로그에서 빠지는 순간 그 주소가 하드 404 가 됐다 — 실측 2026-09-12: 11일 만에 16.8%, 발행된 RSS 링크 185건 중 33건. 원인이 노화가 아니라 **재클러스터링 이동**이라 창을 넓혀도 안 풀린다. 이동은 기사 해시로만 판정하고(제목 유사도 금지), 갱신 시점마다의 제목·요약을 남겨 인용을 되짚게 한다 |
...
| `issue_ledger.json` | 이슈 신원 원장 — issue_id·제목 이력·기사 해시·`moved_to`. 커밋 안 하면 옛 이슈 주소가 다시 404 가 된다. `deploy-web.yml` 에는 커밋 스텝이 없으므로 daily-brief 의 `Commit issue review cache` 스텝이 싣는다 |
