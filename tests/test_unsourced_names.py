...
def test_성이_같아도_원문에_없는_이름이면_줄인_표기로_되돌린다():
    """성이 같다고 맞는 사람은 아니다 — 모델은 학습 시점의 인물로 빈칸을 채운다.

    2026-09-26 실측: 원문 `한 총리, 오는 14일 빌 게이츠 회동`, 출력 '한덕수 국무총리'.
    그날 총리는 한성숙이었다. 예전에는 '李 → 이재명은 확장이 맞다'며 통과시켰다.
    """
    src = "한 총리, 오는 14일 빌 게이츠 회동…SMR 협력 논의 전망"
    got = strip("한덕수 총리가 14일 방한하는 빌 게이츠와 만난다.", src)
    assert "한덕수" not in got, got
    assert "한 총리가" in got, got
    src2 = '李 대통령 "해남 청정에너지, 반도체 클러스터 움직이는 힘"'
    assert strip("이재명 대통령이 착공식에서 밝혔다.", src2) == "이 대통령이 착공식에서 밝혔다."


def test_풀네임이_원문에_있으면_그대로():
    src = '李 대통령 "해남 청정에너지"… 이재명 대통령은 이날 착공식에서'
    got = strip("이재명 대통령이 착공식에서 밝혔다.", src)
    assert "이재명 대통령" in got, got


def test_성과_겹치는_두_글자_낱말은_이름이_아니다():
    src = "李 대통령, 해남 착공식 참석"
    for text in ("이날 대통령은 착공식에 참석했다.", "이번 대통령 일정은"):
        assert strip(text, src) == text, text
