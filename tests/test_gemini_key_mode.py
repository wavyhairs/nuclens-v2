...
class TestWorkflowsSelectTheKey(unittest.TestCase):
    def test_every_gemini_step_uses_the_selection_expression(self):
        ...
        # 16 = 17 - 1. cards.yml 의 "Make story cards" 에서 키를 걷었다
        # (2026-09-20). 그 스텝은 더 이상 LLM 을 부르지 않는다 — 스토리 카피는
        # 앞 "Make cards" 스텝이 같은 편집 판단으로 이미 썼고, 여기서는 렌더와
        # 검증만 한다. 키가 없는 것이 계약이다.
