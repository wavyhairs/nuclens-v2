...
### 운영 콘솔(/admin)은 회차 하나를 연다

병합 진단 화면은 네 계층(같은 날 병합 · 붙이지 않은 판단 · 날짜를 넘는 병합 ·
붙지 않은 경계선)을 한 페이지에 이어 붙여 그렸다. 실측 2026-08-21 라이브에서
**카드 183장**이 한 번에 DOM 에 섰고 그중 그날 새로 볼 것은 스무 장 남짓이었다.
그래서 진입 단위를 **진단 회차**로 바꿨다(`rounds` 색인, `build_admin_merges`).

회차는 기사가 **보도된 날**이 아니라 그 판단이 **내려진 회차**다. 저장된 것이
그 회차의 스냅숏이 아니라서(병합은 빌드마다 전량 재계산된다) 계층마다 역산한다:

| 계층 | 회차를 어디서 얻는가 |
|---|---|
| 같은 날 병합 | `briefing_date` 그 자체(발송 전 수집분은 보도일로 물러난다) |
| 붙이지 않은 판단 | `delivery_log` 의 `story_audit.date` = 발송 회차 |
| 날짜를 넘는 병합 | 합류한 기사의 `briefing_date`(`matches[].hash` ↔ `members[]`) |
| 붙지 않은 경계선 | 카드 경로는 `right_date`(실측 301건 전부 `left_date <= right_date`) |

**역산이 불가능한 자리가 하나 있다.** 미발송 근거 기사(`member_role="evidence"`)는
briefing 회차가 아예 없다 — 그 판단은 과거 어느 날이 아니라 **이번 빌드**가 내린
것이고 다음 빌드도 다시 내린다. 그래서 보도일이 아니라 `build_round` 에 싣고
화면에 *미발송 근거* 배지를 단다. 보도일에 실으면 "8월 12일 진단이 이걸 놓쳤다"는
없는 사실이 생긴다 — 그날의 진단은 그 쌍을 본 적조차 없다.

창은 회차 안에서 **경로마다** 따로 준다(`CONSOLE_BORDERLINE_PER_ROUND = 10`).
근거 쪽이 압도적이라(실측 2026-08-22 빌드 회차 2,053쌍 대 0쌍) 한 창에 세우면
발송 기사 경로가 통째로 사라진다. 전역 상위 N건으로 자르던 옛 규칙은 최신 회차가
창을 독점해 과거 회차의 **건수 자체가 거짓**이 됐다(실측 2026-08-21: 8월 18일
후보 30쌍 중 전역 상위 40건 안에는 1쌍).

#### 경계선의 모수는 검수 밴드다 — 기록 문턱이 아니라

`review_candidates` 는 병합 문턱(0.92)이 아니라 **기록 문턱**(0.70) 위를 전부
담는다. 그걸 그대로 "경계선 후보"로 세면 화면이 "조금만 내리면 붙습니다"라고
적어 놓고 0.75 짜리까지 세는 셈이다 — 2026-08-22 라이브에서 8월 22일 회차가
**29,305건**으로 보였고 그 숫자를 보고 물어서 고쳤다.

그래서 화면에 세우는 것은 `_near_merge_threshold`(= `issue_review.REVIEW_BAND_LOW`
~ `REVIEW_BAND_HIGH`) 안쪽만이다. 같은 날 라이브 실측:

    채점된 쌍 34,263  (0.70~0.80 22,626 · 0.80~0.84 9,436 · 원격 임베딩 없음 1,279)
    밴드 안      922  (2.7%)  ← 화면이 '경계선'이라고 부르는 것
    실제로 실림   99  (회차·경로마다 상위 10쌍)

나머지를 **숨기지는 않는다.** 회차 색인의 `scored` 로 남고 요약에 *채점된 쌍*
칸으로 선다 — 배경 규모를 지우면 "경계선 656쌍"이 그 회차에 벌어진 일의 전부인
것처럼 읽힌다. 회차를 고르는 목록도 같은 이유로 **판단**(같은 날 병합 · 붙이지
않은 판단 · 날짜를 넘는 병합)과 **경계선**을 따로 적는다. 더하면 더미가 판단을
덮는다.

`issue_review.in_review_band` 를 그대로 쓰지 않는다. 그쪽은 'LLM 에게 물을
것인가'를 정하는 자리라 **차단된 쌍을 뺀다.** 콘솔에서는 차단이야말로 볼 것이다
— "차단이 과했나"를 여기서 고르고, 표에 차단 열이 있는 것도 그래서다.

그래서 화면의 숫자는 **언제나 전수**이고 실린 수를 함께 적는다("N건 중 M건").
회차는 **진입 필터일 뿐**이라 카드를 열면 이슈의 전체 맥락(최초·최근 등장일,
전체 멤버, 전체 연결 근거)이 그대로 있다 — 며칠에 걸친 이슈를 날짜 조각으로
쪼개지 않는다(§ 이슈 연속성).

계약은 `web/tests/test_prototype.py::AdminConsoleTests` 의 회차 테스트 8개와
`node web/tests/admin_render.mjs`(회차 이동·접힘 렌더)가 잠근다. 접힌 칸은
**펼칠 때 그리고 접을 때 비운다** — `details` 는 닫혀도 자식을 DOM 에 그대로
들고 있어서 접기만으로는 카드가 한 장도 안 줄기 때문이다.

**흉내 DOM 은 내가 아는 것만 흉내 낸다.** 그래서 `node web/tests/admin_dom.mjs`
가 따로 있다 — 러너·로컬의 크롬을 `--dump-dom` 으로 불러 진짜 문서를 받아 본다
(새 의존성 없음, 배포 경로에 있음). 2026-08-22 배포가 그 값을 치렀다:

    '먼저 확인할 것'의 [해당 칸 열기] 버튼도 data-fold 를 들고 문서에서 details
    보다 앞에 선다 → document.querySelector('[data-fold="story"]') 가 버튼을
    돌려줬고, 버튼에 open=true 는 그냥 임의의 속성 하나다 → 우선 확인에 걸린 칸
    셋이 통째로 안 그려진 채 라이브에 올라갔다(요약 줄 '건수를 세는 중'이 증상).

흉내 DOM 은 선택자를 정규식으로 받아 언제나 그 칸을 돌려줬으므로 초록불이었다.
**선택자의 모호함과 `details` 의 진짜 의미는 실제 문서에만 있다.** 칸을 찾을
때는 `foldBox()` 로 `details[data-fold=…]` 까지 좁힌다.
...
나머지 31,571건은 LLM 도 콘솔(`merges.json` — 회차마다 경로별 상위 10쌍)도
집어 들지 않는다. 즉
...
| 〃 | `admin_gate.mjs` · `admin_render.mjs` · `admin_dom.mjs` | 콘솔 자물쇠 · 흉내 DOM 렌더 · **실물 브라우저 렌더** |
