...
def synthesize_expert(script: str) -> tuple[bytes, int, list[str], list[str]]:
    ...
    **음색보다 완주를 위에 둔다.** 전 구간을 한 모델로 통일해 봤는데(2026-08-16),
    ...
    백오프는 오류 종류로 가른다(2026-09-08 개정). 하루 한도는 기다린다고 회복되지
    않으므로 여전히 즉시 포기하지만, 503 UNAVAILABLE 은 서버가 스스로 "잠시
    뒤 다시"라고 말하는 일시 과부하다 — 여기에까지 백오프를 안 두는 바람에
    그날 4번의 시도가 전부 같은 초에 죽었다. 판정과 대기는 audio_brief.call_tts
    가 하고(총량은 TTS_BACKOFF_BUDGET_SEC 로 묶인다), 이 함수는 완주만 본다.

    음색이 구간 경계에서 달라지는 것은 warnings 로 남긴다.
