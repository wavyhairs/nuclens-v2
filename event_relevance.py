...
INTEREST_TERMS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ...
    ("carbon_free", ("무탄소", "무탄소전원", "cfe", "청정에너지",
                     ...
                     # 2026-09-12 추가. 표에 '무탄소' 는 있는데 **'탄소중립' 이
                     # 없었다** — 한국어에서 이 분야를 부르는 가장 표준적인 말이
                     # 빠져 있었던 셈이다. 실측: '기후산업국제박람회'(9/16~18
                     # 부산 벡스코)와 그에 붙은 '탄소중립 에너지 대전환 포럼'
                     # (9/18)이 둘 다 no_interest_match 로 떨어졌다.
                     #
                     # 이 갈래를 두는 이유는 renewable_grid 와 같다 — 원자력
                     # 그 자체는 아니지만 전원믹스를 두고 같은 자리에서 다투는
                     # 주제라 이 달력이 볼 값이 있다(2026-09-10 사용자 요청).
                     # 중요도 판정을 함께 지나므로 '탄소중립 정책 발표' 같은
                     # 일반 기사는 여기를 통과해도 칸에 서지 않는다.
                     "탄소중립", "탈탄소", "넷제로", "net zero", "net-zero",
                     "기후산업", "기후테크", "climate tech", "decarboni",
                     # 부산 벡스코에서 기후산업국제박람회와 **동시 개최**되는
                     # 국제환경에너지산업전(ENTECH). 전력·발전·수소·신재생을
                     # 전 분야로 다루는 자리다.
                     "환경에너지", "에너지산업전", "에너지대전")),
...
NOISE_TERMS = ("채용", "공채", "신입 공채", "신입사원", "경력사원", "교육생 모집", "수강생 모집",
               ...
               # 금융 상품 판매. 주제어(데이터센터·전력)를 담고 마감일까지 있어
               # 형식 판정을 통과하지만 정책·산업 일정이 아니다 — 실측
               # 2026-09-15: 'AI 병목 해소 테마 목표전환형 펀드 모집'.
               "펀드", "수익증권", "청약", "공모주", "상장지수",
               "홈페이지 점검", "서버 점검", "휴무", "홍보")
