...
- title_kr: 한국어 **뉴스 헤드라인** (30~60자). 원문을 옮긴 번역문이 아니라 헤드라인을 쓴다 —
  카드를 열지 않아도 무슨 일이 있었는지 알 수 있어야 한다.
  · 형태는 `행위 주체 + 핵심 행동`. 주체를 지우고 현상만 적지 말 것.
    명사 나열로 끝내지 말 것 — `~관련 동향`·`~관련 논의`·`~중요성 부상`·`~필요성 제기`·`~우려 확산`
    은 무슨 일이 있었는지 말하지 않는다. (나쁨: `AI 데이터센터 전력 수요 급증과 공급난 우려`
    / 좋음: `IEA, 2028년 데이터센터 전력수요 2배 전망`)
  · **사건 규모를 가늠하게 하는 수치는 우선 보존한다** — 용량(GW·MW)·금액·기수·증감률·계약기간·
    시행시점. 원문에 있는 수치를 이유 없이 버리지 말 것.
    (나쁨: `러시아, 원전 건설 확대 추진` / 좋음: `러시아, 2042년까지 원전 38기 건설 추진`)
  · 원제목이 클릭베이트이거나(`진짜 병목은 OO?`·`유독 싸다더니`) 핵심을 숨기면 **본문 근거로 다시 쓴다.**
    말머리(`[이슈]`·`[영상]`·`[○○ 칼럼]`)와 매체 브랜드는 뺀다.
  · **사건 단계를 정확히 유지한다** — 계획·검토·추진·협의·합의·계약·승인·착공·준공은 서로 다른 사건이다.
    원문이 `검토`면 `결정`으로, `추진`이면 `확정`으로, 전망이면 결정으로 올려 쓰지 말 것.
  · **배경 사실을 오늘 사건처럼 올리지 않는다** — 제목은 이 기사가 새로 전하는 소식이다.
    기획·해설·칼럼·인터뷰·르포는 기사의 논지·질문을 제목으로 쓴다. 본문에 배경으로 나온
    과거 사건(지난해 계약, 지난달 사고, 이미 나온 발표)을 방금 일어난 일처럼 제목에 올리지 말 것.
    그 사실을 꼭 써야 하면 시점을 제목에 밝힌다.
    (나쁨: 창간기획 `냉각이냐 전력이냐…LG전자·두산에너빌` → `두산에너빌리티, 한수원과 체코 원전 주기기 계약 체결`
     — 계약은 지난해 12월 일이다 / 좋음: `냉각이냐 전력이냐…LG전자·두산에너빌리티 데이터센터 수익화 경쟁`)
    (나쁨: `하나로 원자로, 출력 증가 중 자동정지 사건 발생` — 정지는 3월 일이다
     / 좋음: `하나로 원자로 3월 자동정지…작년부터 이상신호 있었다`)
  · **주장은 쓴 사람의 것이다** — 칼럼·기고·사설의 주장을 정부·지자체·기관의 조치로 바꿔 쓰지 말 것.
    (나쁨: `[칼럼] 울산 SMR 특구, 부지 경쟁 말고 산업 경쟁을` → `울산시, SMR 특구 전략 모색`)
  · **위험과 영향을 부풀리지 않는다** — `차질`·`무산`·`중단`·`좌초`·`위기`는 원문이 그렇게 말할 때만 쓴다.
    원문이 `우려`·`불확실성`·`지연 가능성`이면 그 말을 그대로 쓴다.
  · **성만 나온 인물을 풀어 쓰지 않는다** — 원문이 `한 총리`·`李 대통령`처럼 성만 쓰면 그대로 둔다.
    원문에 없는 이름을 채우면 학습 시점의 다른 사람이 들어간다(실측: `한 총리` → `한덕수`, 그날 총리는 한성숙).
  · 인명·기관명 첫 등장 시 한글(원문) 병기. 원문이 영문이면 한국어로 옮긴다.
  · **원문에 없는 것은 넣지 않는다** — 인물명·기업명·기관명·수치 어느 것도 지어내지 말 것.
    정보밀도를 높이라는 것은 **원문에 이미 있는 사실을 제목으로 끌어올리라**는 뜻이지
    아는 사실로 채우라는 뜻이 아니다. `본문:` 이 없는 기사는 제목이 말하는 범위 안에서만 다시 쓴다
    (아래 summary 의 본문 없는 기사 규칙과 같다).
...
- implication: AI 해석 1문장(90자 이내). nice_to_know·must_read만 작성. 완결형 서술문으로 쓰고 문자열을 자르지 말 것.
  ...
  **상투적 대응 권고도 금지다** — "적극 대응할 필요가 있다"·"면밀한 모니터링이 필요하다"·
  "관련 전략을 마련해야 한다"는 어느 기사에나 붙는 말이라 정보가 0이다. 대신 **어떤
  제도·사업·지표를 봐야 하는지**를 이름으로 지목할 것.
  **수혜 산업 추론도 금지다.** 원문이 그렇게 말하지 않았는데 "국내 ○○업계에 기회가
  될 수 있다"·"한국 기업들의 수출 확대가 기대된다"처럼 수혜 업종·수혜 기업을 짚지 말 것.
  이 브리핑은 투자 뉴스가 아니다.
    나쁨: "대규모 송전망 확충 정책은 초고압 전력기기 기술력을 보유한 한국 기업들에
          새로운 해외 시장 진출 기회가 될 수 있다." (원문에 한국 기업이 없다)
    좋음: "2035년까지 120GW 라는 목표가 기존 계획의 2배로, 계통 보강 일정이 함께
          제시됐는지가 실현 가능성의 관건이다." 
...
- why_important: must_read만 작성. **1~2개의 완결형 문장, 150자 이내**. 분석관 톤. 격식체. 절대 길게 풀어쓰거나 문자열을 자르지 말 것.
  **이 필드는 '의미 부여'가 아니라 '상태 변화'를 쓴다.** `기존에 무엇이었는가 → 이번에 무엇이
  바뀌었는가 → 앞으로 무엇을 보면 되는가` 중 최소 둘을 담을 것. 특히 사건 단계가 어디에서
  어디로 넘어갔는지(검토→추진→계획→결정→승인→계약→착공→준공→가동)를 짚으면 가장 좋다.
  아래처럼 끝나는 문장은 금지다 — 무엇이 달라졌는지 말하지 않는다(implication 의 금지 목록과 같다):
  "…이 중요하다 / …에 중요한 의미가 있다 / …중요한 사례다 / …영향을 미칠 수 있다 /
   …관심이 필요하다 / …이 요구된다 / …이 전망된다 / …을 시사한다 / …을 보여준다".
    나쁨: "정부의 원전 정책 변화가 원자력 산업에 중요한 의미가 있다." (제목을 바꿔 말했을 뿐)
    좋음: "정부 검토 단계였던 신규 원전 지원이 실제 예산 프로그램으로 전환돼 사업 추진 단계가 한 단계 진전됐다."
    좋음: "고정 송전용량 방식 대신 계절별 용량 산정을 실제 계통에 시험하는 첫 단계로, 향후 제도화 여부가 핵심이다."
  담을 변화가 기사에 없으면 **빈 문자열로 둔다.** 억지로 채우지 말 것.
...
- event_date: 제목이 전하는 사건의 발생·발표·시행·예정일을 YYYY-MM-DD로 작성. `지난 22일`·`지난달 31일`·`지난해 12월`처럼 상대 날짜는 입력의 `게재일:` 기준으로 풀어 쓴다(월만 알면 event_date_precision=month 로 그 달 1일). 기사 게시일을 사건일로 추정하지 말 것. 일자를 확정할 수 없으면 null.
    ...
    **학습 시점의 대통령**으로 메운다. 무작위 오타가 아니라 방향이 정해진 오류라
    ...
    def replace(match: re.Match) -> str:
        ...
        if name[0] in surnames:
            # 성은 맞는데 이름은 원문에 없다 — 모델이 **학습 시점의 인물**로 빈칸을
            # 채운 것일 수 있다. 성이 같다고 맞는 사람은 아니다.
            #
            # 실측 2026-09-26 (연합뉴스TV, 원문 `한 총리, 오는 14일 빌 게이츠 회동`):
            # 큐레이션이 '한덕수 국무총리'로 풀어 썼는데 그날 총리는 한성숙이었다.
            # 처음에는 '李→이재명은 풀어 쓴 게 맞다'로 통과시켰지만, 맞는 날이 있는
            # 것과 규칙이 맞는 것은 다르다. 원문이 쓴 줄인 표기('한 총리')로 되돌린다.
            #
            # 3음절 이상만 본다. '이날 대통령'·'신임 장관'·'한편 총리'처럼 첫 글자가
            # 성과 겹치는 두 글자 낱말을 이름으로 오인하면 문장이 바뀐다.
            if len(name) < 3:
                return match.group(0)
            UNSOURCED_NAME_DROPS.append(
                f"{where[:40]} | 원문 {''.join(surnames)} {title} ↔ 출력 {name} {title}(원문에 없는 이름)")
...
def curate_batch(articles: list[dict], reports_kb: list[dict],
    ...
    def run_chunk(chunk: list[dict], error_notes: dict[str, list[str]] | None = None,
        ...
        for i, art in enumerate(chunk):
            ...
            # 게재일. 없으면 모델이 '지난 22일'·'지난해 12월'을 풀 기준이 없어 event_date 가
            # 비고(2026-09 아카이브 90% null), 배경으로 나온 옛 사건을 새 소식처럼 제목에
            # 올린다(2026-09-26 발송분 점검: 옛 사실을 새 일처럼 쓴 제목 17건).
            if isinstance(art.get("pub"), datetime):
                lines.append(f"게재일: {art['pub'].astimezone(KST).date().isoformat()}")
